...
---
## <a name="noA12d1tjC" id="noA12d1tjC">Bytecodes</a>

### 概要(Summary)
HotSpot 内部で使用するバイトコードの一覧を表すクラス (より正確には, そのための機能/定数値を納めた名前空間(AllStatic クラス)).

なお HotSpot 内部では, Java 仮想マシン仕様で定められている 203 種類のバイトコード以外に, 独自のバイトコードも定義されている.
これらは現状では 203 番以降の領域に割り当てられている. (See: [here](no7882vBO.html) for details)
...
(ここまでが JVM 仕様標準のバイトコード)

    
        number_of_java_codes,
    

(ここから先が HotSpot 独自のバイトコード)
...
(プラットフォーム毎に独自のバイトコードを追加することもできる)
...
(全バイトコード数は number_of_codes で参照できる)
...
### 詳細(Details)
See: [here](../doxygen/classBytecodes.html) for details
